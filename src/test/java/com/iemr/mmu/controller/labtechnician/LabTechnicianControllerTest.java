...
//	
	...
	void testGetBeneficiaryPrescribedProcedure() throws Exception {
		...
		String requestOBJ = "{\"benRegID\":\"1\",\"visitCode\":\"1\"}";
		String s = "Test";

		JsonObject jsnOBJ = parseJsonRequest(requestOBJ);

		// when(labTechnicianServiceImpl.getBenePrescribedProcedureDetails(1L,
		// 1L)).thenReturn(s);
	...
	void testGetLabResultForVisitCode_Success() throws Exception {
		...
		String requestOBJ = "{\"BENEFICIARY_REG_ID\": \"12345\", \"VISIT_CODE\": \"67890\"}";
		String s = "test";
		JsonObject jsnOBJ = parseJsonRequest(requestOBJ);

		// when(labTechnicianServiceImpl.getLabResultForVisitcode(anyLong(),
		// anyLong())).thenReturn(s);
	...
	@Test
	void testGetLabResultForVisitCode_MissingParameters_ReturnsError() {
		// Arrange
		String requestOBJ = "{\"BENEFICIARY_REG_ID\": \"12345\"}"; // VISIT_CODE missing

		// Act
		String result = labTechnicianController.getLabResultForVisitCode(requestOBJ);

		// Assert
		assertThat(result).contains("Invalid request");
	}

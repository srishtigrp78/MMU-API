...
public interface LabTestMasterRepo extends CrudRepository<LabTestMaster, Integer> {
	@Query("SELECT testID, testName, isRadiologyImaging FROM LabTestMaster c where c.deleted != 1")
	...
	@Query("SELECT testID, testName FROM LabTestMaster c where c.testFor=:testFor AND c.deleted != 1")

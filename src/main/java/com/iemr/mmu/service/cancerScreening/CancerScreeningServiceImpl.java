...
public class CancerScreeningServiceImpl implements CancerScreeningService {
	...
	public int UpdateCSHistoryNurseData(JsonObject jsnOBJ) {
		...
		if(jsnOBJ.has("historyDetails") && null != jsnOBJ.get("historyDetails")){
			...
			if(null!= historyDetails && historyDetails.has("familyHistory") && !historyDetails.get("familyHistory").isJsonNull()){
				JsonObject familyHistory = historyDetails.get("familyHistory").getAsJsonObject();
				BenFamilyCancerHistory[] benFamilyCancerHistoryArray = InputMapper.gson().fromJson(familyHistory.get("diseases"),
			...
			if(null!= historyDetails && historyDetails.has("pastObstetricHistory") && !historyDetails.get("pastObstetricHistory").isJsonNull()){
				
				BenObstetricCancerHistory benObstetricCancerHistory = InputMapper.gson().fromJson(historyDetails.get("pastObstetricHistory"),
						BenObstetricCancerHistory.class);
				pastObstetricCURes = updateBenObstetricCancerHistory(benObstetricCancerHistory);
			}
			
			if(null!= historyDetails && historyDetails.has("personalHistory") && !historyDetails.get("personalHistory").isJsonNull()){ 
				
				BenPersonalCancerHistory benPersonalCancerHistory = InputMapper.gson().fromJson(historyDetails.get("personalHistory"),
						BenPersonalCancerHistory.class);

				BenPersonalCancerDietHistory benPersonalCancerDietHistory = InputMapper.gson().fromJson(historyDetails.get("personalHistory"),

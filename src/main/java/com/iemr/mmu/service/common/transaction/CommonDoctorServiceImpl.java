...
public class CommonDoctorServiceImpl {
	...
	public String getFindingsDetails(Long beneficiaryRegID, Long benVisitID) {
		ArrayList<Object[]> clinicalObservationsList = benClinicalObservationsRepo.getFindingsData(beneficiaryRegID, benVisitID);
		ArrayList<Object[]> chiefComplaintsList = benChiefComplaintRepo.getBenChiefComplaints(beneficiaryRegID, benVisitID);
	...
	public String getPrescribedDrugs(Long beneficiaryRegID, Long benVisitID) {
		...
		if(null != prescriptionsList && prescriptionsList.size()>0){
			for(PrescriptionDetail prescription : prescriptionsList){
				ArrayList<Object[]> prescribedDrugs = prescribedDrugDetailRepo.getBenPrescribedDrugDetails(prescription.getPrescriptionID());

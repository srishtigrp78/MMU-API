...
public class PNCNurseServiceImpl implements PNCNurseService{
	...
	public Long saveBenPncCareDetails(PNCCare pncCareOBJ) throws ParseException {
		...
		if (pncCareOBJ.getdDate() != null && !pncCareOBJ.getdDate().isEmpty()
				&& pncCareOBJ.getdDate().length() >= 10) {
			String dDate = pncCareOBJ.getdDate().split("T")[0];
			pncCareOBJ
					.setDateOfDelivery(new Date(new SimpleDateFormat("yyyy-MM-dd").parse(dDate).getTime()));
		}
	...
	public int updateBenPNCCare(PNCCare pncCareOBJ) throws ParseException {
		...
		r = pncCareRepo.updatePNCCareDetails(pncCareOBJ.getDeliveryTypeID(),pncCareOBJ.getDeliveryType(),
				pncCareOBJ.getDeliveryPlaceID(), pncCareOBJ.getDeliveryPlace(),
				pncCareOBJ.getOtherDeliveryPlace(), pncCareOBJ.getDateOfDelivery(), 
				pncCareOBJ.getDeliveryComplicationID(), pncCareOBJ.getDeliveryComplication(), 
				pncCareOBJ.getOtherDeliveryComplication(),  pncCareOBJ.getPregOutcomeID(), 
				pncCareOBJ.getPregOutcome(), pncCareOBJ.getPostNatalComplicationID(),
